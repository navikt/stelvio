 ...
 * SDOInspector sdoppt = new SDOInspector(2,"-");
 * 
 * Alternative 1:
...
public class SDOFormatter {
	...
	private static Logger m_logger = Logger.getLogger(SDOFormatter.class.getName());
	 ...
	 */
	public SDOFormatter(int indent, String indentString) {
	...
	public void sdoPrettyPrint(DataObject sdo, PrintWriter out) {
		...
		printDataObjectShort(sdo, null, out, 0, baseIndentStr);
	...
	protected void printDataObject(DataObject sdo, String instanceName,
		...
		out.print(sep + " " + (instanceName != null ? instanceName : sdoName)
	...
	/**
	 * This method perform the same operation as <code>printDataObject(..)</code>,
	 * but will not include the properties datatype or URI (i.e. [String : commonj.sdo])
	 * 
	 * NB! This method will neigther print empty properties!
	 * 
	 * @param sdo The data object to print
	 * @param instanceName 
	 * @param out The PrintWriter to use to print the data object
	 * @param indent The size of the indent for the current level
	 * @param indentStr The string to use for indentation
	 */
	protected void printDataObjectShort(DataObject sdo, String instanceName,
			PrintWriter out, int indent, String indentStr) {
		
		m_logger.entering(
				"SDOInspector",
				"printDataObjectShort(DataObject sdo, PrintWriter out, int indent, String indentStr)");
		Type sdoType = sdo.getType();
		String sep = getIndentStr(indent, indentStr);
		String output = instanceName;
		
		if (instanceName == null)
		 output = sdoType.getName();
		
		out.println(sep + " " + output);
		List sdoPropertyList = sdoType.getProperties();
		Iterator propIterator = sdoPropertyList.iterator();
		while (propIterator.hasNext()) {
			Property property = (Property) propIterator.next();
	...
	protected void printObject(Object child, String name, Property prop,
		...
		if (child instanceof DataObject) {
			printDataObjectShort((DataObject) child, name, out, indent, indentStr);
