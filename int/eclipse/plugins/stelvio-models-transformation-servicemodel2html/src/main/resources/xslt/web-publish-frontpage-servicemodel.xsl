...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="no" lang="no">
			<head>
				<title>NAV Tjenestebuss - tjenestespesifikasjoner</title>
					...
					<div class="colmid">
						<div class="colleft">
							...
							<div class="col1">
								...
								<div id="tjenestemodel-container">
									<ul>
										<li class="package">no.nav
											<xsl:apply-templates select="//childPackages[../@name='no.nav' and ../../@name = 'tjenestemodell']" mode="servicemodel"/>
										</li>
										<li class="package">no
											<xsl:apply-templates select="//childPackages[../@name='no' and ../../@name = 'tjenestemodell']" mode="servicemodel"/>
										</li>
									</ul>
								</div>
							</div>
	...
	<xsl:template match="serviceInterface">
		<ul>
			<li class="serviceInterface"><xsl:value-of select="@name" />
				<ul>
					<xsl:apply-templates select="./serviceOperations" mode="createLink"/>
				</ul>
			</li>
		</ul>
	</xsl:template>	
