...
import no.nav.bpchelper.utils.ConnectorProperties;
 ...
 * 
 * Since an instance of the <code>BFMConnection</code> class is 
 * needed to make use of the API, and the its contructurs don't
 * handle with secuity must this class encapsulate that.
 * 
 * @author Andreas Roe
 */
public class SecuredBFMConnection extends BFMConnection {
	...
	public BusinessFlowManagerService getBusinessFlowManagerService() throws NamingException, CreateException, RemoteException {
		...
		String username = properties.getProperty(ConnectorProperties.USERNAME);
		String password = properties.getProperty(ConnectorProperties.PASSWORD);
		...
		BusinessFlowManager bfm = ((BusinessFlowManagerHome) PortableRemoteObject
				.narrow(ctx.lookup(getJndiName()), BusinessFlowManagerHome.class))
				.create();
