...
def __sca():
	return [ScaModule(DEFAULT_MODULE_NAME, '0.0.0', 1), ScaModule(DEFAULT_MODULE_NAME, 2)]
cbg.getInstalledModules = __sca

scaModule1 = ScaModule(DEFAULT_MODULE_NAME, '0.0.0', 1)
scaModule2 = ScaModule(DEFAULT_MODULE_NAME, '0.0.0', 2)
...
def getIntersectionTest():
	...
	scaModules = [ScaModule('pensjon-tjeneste-beregning', '0.0.0', 1), ScaModule('pensjon-tjeneste-beregning', '0.0.0', 2), ScaModule('nav-tjeneste-oppgave', '0.0.0', 2), ScaModule('nav-ent-pen-grunnlag', '0.0.0', None), ScaModule('nav-ent-pen-krav', '0.0.0', None)]
	intersection= cbg.getIntersection(blaModules, scaModules)
	assertContains(intersection, ScaModule('nav-ent-pen-grunnlag', '0.0.0', None), 'Did not find the expected module in the list!')
