...
public class CommandOptionsBuilder {
	...
	private Option getTimeFrameOption() {
		...
		timeFrame.setDescription("Filter by time. Pattern:" + no.nav.sibushelper.common.Constants.TIME_FRAME_FORMAT + "-"
				+ no.nav.sibushelper.common.Constants.TIME_FRAME_FORMAT);
		return timeFrame;
	}

	/**
	 * Create command line option for 'maxResultSet'. This option is not
	 * required
	 * 
	 * @return
	...
	private Option getMaxResultSetOption() {
		...
		maxResultSet
				.setDescription("Default value is '100'. Recommended to use range between 100 and 1000. This is a restriction due to DB anf JVM restrictions!");
		return maxResultSet;
	}
