...
public class FrameworkTilesRequestProcessor extends TilesRequestProcessor {
	...
	protected boolean processValidate(
		...
		String method = no.trygdeetaten.web.framework.util.RequestUtils.getMethod(request);

		if (no.trygdeetaten.web.framework.util.RequestUtils.validate(strutsParameter, method)) {
	...
	protected void processForwardConfig(HttpServletRequest request, HttpServletResponse response, ForwardConfig forward)
		...
		if (!(forward instanceof SessionScope)) {
			...
			no.trygdeetaten.web.framework.util.RequestUtils.removeSessionAttribute(request, mapping.getName());
			if (log.isDebugEnabled()) {
		...
		TransactionContext.setModuleId(no.trygdeetaten.web.framework.util.RequestUtils.getScreenId(uri));
