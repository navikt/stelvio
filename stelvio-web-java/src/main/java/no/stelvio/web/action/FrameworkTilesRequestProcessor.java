...
public class FrameworkTilesRequestProcessor extends TilesRequestProcessor {
	...
	protected boolean processValidate(
		...
		String method = no.stelvio.web.util.RequestUtils.getMethod(request);

		if (no.stelvio.web.util.RequestUtils.validate(strutsParameter, method)) {
	...
	protected void processForwardConfig(HttpServletRequest request, HttpServletResponse response, ForwardConfig forward)
		...
		if (!(forward instanceof SessionScope)) {
			...
			no.stelvio.web.util.RequestUtils.removeSessionAttribute(request, mapping.getName());
			if (log.isDebugEnabled()) {
		...
		RequestContext.setModuleId(no.stelvio.web.util.RequestUtils.getScreenId(uri));
