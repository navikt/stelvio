...
<xsl:stylesheet
  ...
  <xsl:template match="body"  mode="map:HentPersonUtland">
    <body>
      ...
      <xsl:apply-templates select="in2:hentPerson" mode="localHentPersonToHentTPSData_1345592706"/>
    </body>
  </xsl:template>
  ...
  <xsl:template name="map:HentPersonUtland2">
    ...
    <xsl:apply-templates select="$body/in2:hentPerson" mode="localHentPersonToHentTPSData_1345592706"/>
  </xsl:template>

  <!-- This rule represents an element mapping: "in2:hentPerson" to "out2:hentTPSData".  -->
  <xsl:template match="in2:hentPerson"  mode="localHentPersonToHentTPSData_1345592706">
    <out2:hentTPSData>
      <!-- a structural mapping: "request"(HentPersonRequest) to "tpsPersonDataReq"(tpsPersonDataType) -->
      <xsl:apply-templates select="request" mode="localRequestToTpsPersonDataReq_1123368124"/>
    </out2:hentTPSData>
  </xsl:template>

  <!-- This rule represents an element mapping: "request" to "tpsPersonDataReq".  -->
  <xsl:template match="request"  mode="localRequestToTpsPersonDataReq_1123368124">
    <tpsPersonDataReq>
      <tpsServiceRutine>
