...
<xsl:stylesheet
  ...
  <xsl:template match="/">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io2:smo" mode="map:MapFaultPersonIkkeFunnet"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io2:smo" to "io2:smo".  -->
  <xsl:template match="io2:smo"  mode="map:MapFaultPersonIkkeFunnet">
    <io2:smo>
      <!-- a structural mapping: "context"(<Anonymous>) to "body"(hentPerson_PersonIkkeFunnet) -->
      <xsl:apply-templates select="context" mode="localContextToBody_48796478"/>
    </io2:smo>
  </xsl:template>
  ...
  <xsl:template name="map:MapFaultPersonIkkeFunnet2">
    ...
    <xsl:apply-templates select="$smo/context" mode="localContextToBody_48796478"/>
  </xsl:template>

  <!-- This rule represents an element mapping: "context" to "body".  -->
  <xsl:template match="context"  mode="localContextToBody_48796478">
    <body>
      <xsl:attribute name="xsi:type">
        <xsl:value-of select="'out4:hentPerson_PersonIkkeFunnet'"/>
      </xsl:attribute>
      <!-- a structural mapping: "shared"(TPSPersonSharedContext) to "out2:hentPersonPersonIkkeFunnet"(PersonIkkeFunnet) -->
      <xsl:apply-templates select="shared" mode="localSharedToHentPersonPersonIkkeFunnet_2118062853"/>
    </body>
  </xsl:template>

  <!-- This rule represents an element mapping: "shared" to "out2:hentPersonPersonIkkeFunnet".  -->
  <xsl:template match="shared"  mode="localSharedToHentPersonPersonIkkeFunnet_2118062853">
    <out2:hentPersonPersonIkkeFunnet>
