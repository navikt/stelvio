...
public class _EchoHome_Stub extends Stub implements EchoHome {
    
    private static final String[] _type_ids = {
        "RMI:ejbs.EchoHome:0000000000000000", 
    ...
    public Echo create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (Echo) in.read_Object(Echo.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",ejbs.EchoHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    Echo result = ((ejbs.EchoHome)so.servant).create();
                    return (Echo)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
