...
public class _LargeGraphHome_Stub extends Stub implements LargeGraphHome {
    
    private static final String[] _type_ids = {
        "RMI:ejbs.LargeGraphHome:0000000000000000", 
    ...
    public LargeGraph create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (LargeGraph) in.read_Object(LargeGraph.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",ejbs.LargeGraphHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    LargeGraph result = ((ejbs.LargeGraphHome)so.servant).create();
                    return (LargeGraph)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
