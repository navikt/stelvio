...
/*
 * <p> This is a utility class that provides helper methods around SDOs, useful
 * for different purpose as copy the SDO properties from Source to Target or
 * other funtionality around SDO </p>
 * 
 * @usage <p>
 * 
 * </p>
...
public class DataObjectUtil {
	 ...
	 */
	public static DataObject deepCopyDataObjectProperties(DataObject source,
			DataObject target) {
			...
			for (Iterator i = source.getType().getProperties().iterator(); i.hasNext();) {
				...
				if (object != null) {
					if (object instanceof DataObject) {
						object = deepCopyDataObjectProperties(
	 ...
	 *            The target SDO object
	 */
	public static DataObject augmentDataObject(DataObject source,
			DataObject target) {
		if (source != null) {
			if (target == null) {
			...
			for (Iterator i = source.getType().getProperties().iterator(); i.hasNext();) {
				...
				if (object != null) {
					if (object instanceof DataObject) {
						object = augmentDataObject((DataObject) object,
								(DataObject) target.get(property));
					}
					target.set(property, object);
				}
			}
	...
	/**
	 * Copies hierachical the value of each property in source to the same property in target if the condition function called returns true.
	 * It does not clone the properties values.
	 * It creates property objects if they don't already exist and the parameter is set to true.
	 * List items are replaced, and additional items are created if createProps is set to true.
	 * @param source The source data object to copy the data from
	 * @param target The target object to copy the data to, will be modified
	 * @param exactProps	Set this to EXACT_TPYE if a property's value should only be copied if it's type is exactly the same than the target property's type;
	 * 						if set to MATCH_NAME a property's value will also be copied if only the property's name, isContainment flag, and isMany flag are matching;
	 * 						if set to MATCH_TYPENAME a property's value will also be copied if only the property's type name, isContainment flag, and isMany flag are matching;
	 * 						if set to MATCH_NAME|MATCH_TYPENAME a property's value will also be copied if only the property's name, type name, isContainment flag, and isMany flag are matching
	 * @param createProps Set this to true to create DataObjects for empty properties
	...
	public static DataObject deepCopyDataObject(DataObject source, DataObject target, int exactProps, boolean createProps, IAugmentCondition condition) {
		if(source!=null) {
			if((target!=null)||createProps) {
				if(target==null) target=DataFactory.INSTANCE.create(source.getType());
				for(Iterator i=source.getType().getProperties().iterator();i.hasNext();) { //iterate over source properties
					...
					if((exactProps&(MATCH_NAME|MATCH_TYPENAME))<=0) {
						...
						canHoldProp=hasProperty(target,((exactProps&MATCH_NAME)>0)?sProp.getName():null,((exactProps&MATCH_TYPENAME)>0)?sProp.getType().getName():null,new Boolean(sProp.isContainment()),new Boolean(sProp.isMany()));
						...
						if(condition.evaluate(source,tProp,target)) { //condition
							if(object==null) { //is null
								...
								if(!sProp.isContainment()) { //is a simple type
									...
									target.set(tProp,deepCopyDataObject((DataObject)object,(DataObject)target.get(tProp),exactProps,createProps,condition));
								}
							} else { //is an array
								if(createProps&&(target.getList(tProp)==null)) target.set(tProp,new Vector());
									...
									for(int j=0;j<sl.size();j++) { //iterate over source array items
										if(j<tl.size()) { //replace item
											if(!sProp.isContainment()) { //simple type
												...
												tl.set(j,deepCopyDataObject((DataObject)sl.get(j),(DataObject)tl.get(j),exactProps,createProps,condition));
											}
										} else if(createProps) { //append item
											if(!sProp.isContainment()) { //simple type
												tl.add(sl.get(j));
											} else { //DataObject
												tl.add(deepCopyDataObject((DataObject)sl.get(j),null,exactProps,true,condition));
											}
										}
									}
								}
							}
							if(!source.isSet(sProp)) target.unset(tProp);
						}
					}
				}
			}
	 ...
	 * </p>
	 * @param source The source data object to copy the data from
	 * @param target The target object to copy the data to, can also be null
	 * @return returns the target object
	 */
	...
	/**
	 * <p> 
	 * Copies hierachical the value of each property in source to the same property in target if the condition function isSetCondition returns true.
	 * It creates property objects if they don't already exist.
	 * </p>
	 * @param source The source data object to copy the data from
	 * @param target The target object to copy the data to, can also be null
	 * @return returns the target object
	 ...
	 * </p>
	 * @param source The source data object to copy the data from
	 * @param target The target object to copy the data to
	 * @return returns the target object
	 */
	...
	public static String convertDataObjectToXML(DataObject sdo) {
        ...
        try {
            BOXMLSerializer xmlSerializer = (BOXMLSerializer) new ServiceManager().locateService("com/ibm/websphere/bo/BOXMLSerializer");
	 ...
	 * </p> 
	 * @param bo The DataObject to search in
	 * @param nameRegex The name of the property to search for, can be a regular expression, can be null to disable this criteria
	 * @param typenameRegex The name of the property's type to search for, can be a regular expression, can be null to disable this criteria
	 * @param isBo Criteria to search for either a DataObject or a simple type, can be null to disable this criteria
	 * @param isArray Criteria to search for either an Array or a flat type, can be null to disable this criteria
	...
	protected static boolean hasProperty(DataObject bo, String nameRegex, String typenameRegex, Boolean isBo, Boolean isArray) {
		for(Iterator i=bo.getType().getProperties().iterator();i.hasNext();) { //iterate over bo properties
			Property property=(Property)i.next();
			if (nameRegex!=null?property.getName().matches(nameRegex):true &&
				typenameRegex!=null?property.getType().getName().matches(typenameRegex):true &&
				isBo!=null?property.isContainment()==isBo.booleanValue():true &&
				isArray!=null?property.isMany()==isArray.booleanValue():true) return true;
		}
		return false;
	}
