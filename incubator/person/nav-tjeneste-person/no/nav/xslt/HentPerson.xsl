...
<xsl:stylesheet
  ...
  <xsl:template match="body"  mode="map:HentPerson">
    <body>
      ...
      <xsl:apply-templates select="in2:hentPerson" mode="localHentPersonToHentTPSData_873551277"/>
    </body>
  </xsl:template>
  ...
  <xsl:template name="map:HentPerson2">
    ...
    <xsl:apply-templates select="$body/in2:hentPerson" mode="localHentPersonToHentTPSData_873551277"/>
  </xsl:template>

  <!-- This rule represents an element mapping: "in2:hentPerson" to "out2:hentTPSData".  -->
  <xsl:template match="in2:hentPerson"  mode="localHentPersonToHentTPSData_873551277">
    <out2:hentTPSData>
      <!-- a structural mapping: "request"(HentPersonRequest) to "tpsPersonDataReq"(tpsPersonDataType) -->
      <xsl:apply-templates select="request" mode="localRequestToTpsPersonDataReq_1979849973"/>
    </out2:hentTPSData>
  </xsl:template>

  <!-- This rule represents an element mapping: "request" to "tpsPersonDataReq".  -->
  <xsl:template match="request"  mode="localRequestToTpsPersonDataReq_1979849973">
    <tpsPersonDataReq>
      <tpsServiceRutine>
