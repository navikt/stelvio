...
<!-- @generated mapFile="no/nav/xslt/HentPerson.map" md5sum="593df40f6685b0a58212f31c822dd2cf" version="7.0.302" -->
<!--
*****************************************************************************
*   This file has been generated by the IBM XML Mapping Editor V7.0.302
*
*   Mapping file:		HentPerson.map
*   Map declaration(s):	HentPerson
...
<xsl:stylesheet
    ...
    xmlns:map="http://nav-tjeneste-person/no/nav/xslt/HentPerson"
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="msl:datamap">
        <msl:datamap>
          <dataObject>
            ...
            <xsl:call-template name="map:HentPerson2">
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="body" mode="map:HentPerson"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "body" to "body".  -->
  <xsl:template match="body"  mode="map:HentPerson">
    <body>
      <xsl:attribute name="xsi:type">
        <xsl:value-of select="'out3:hentTPSDataRequestMsg'"/>
      </xsl:attribute>
      <!-- a structural mapping: "in2:hentPerson"(<Anonymous>) to "out2:hentTPSData"(<Anonymous>) -->
      <xsl:apply-templates select="in2:hentPerson" mode="localHentPersonToHentTPSData_1890090379"/>
    </body>
  </xsl:template>

  <!-- This rule represents a type mapping: "body" to "body".  -->
  <xsl:template name="map:HentPerson2">
    <xsl:param name="body"/>
    <!-- a structural mapping: "$body/in2:hentPerson"(<Anonymous>) to "out2:hentTPSData"(<Anonymous>) -->
    <xsl:apply-templates select="$body/in2:hentPerson" mode="localHentPersonToHentTPSData_1890090379"/>
  </xsl:template>

  <!-- This rule represents an element mapping: "in2:hentPerson" to "out2:hentTPSData".  -->
  <xsl:template match="in2:hentPerson"  mode="localHentPersonToHentTPSData_1890090379">
    <out2:hentTPSData>
      <!-- a structural mapping: "request"(HentPersonRequest) to "tpsPersonDataReq"(tpsPersonDataType) -->
      <xsl:apply-templates select="request" mode="localRequestToTpsPersonDataReq_328977947"/>
    </out2:hentTPSData>
  </xsl:template>

  <!-- This rule represents an element mapping: "request" to "tpsPersonDataReq".  -->
  <xsl:template match="request"  mode="localRequestToTpsPersonDataReq_328977947">
    <tpsPersonDataReq>
      <tpsServiceRutine>
        <!-- a simple mapping with no associated source:  to "serviceRutinenavn"(<SRnavn>) -->
        <serviceRutinenavn>
          <xsl:text>FS03-FDNUMMER-PERSDATA-O </xsl:text>
        </serviceRutinenavn>
