...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:env="http://www.w3.org/2003/05/soap-envelope" xmlns:dp="http://www.datapower.com/extensions" xmlns:str="http://exslt.org/strings" xmlns:date="http://exslt.org/dates-and-times" xmlns:dputils="http://nav.no/datapower-utils" xmlns:dpconfig="http://www.datapower.com/param/config" extension-element-prefixes="dp date" exclude-result-prefixes="dp dpconfig dputils date env str xsd">
	...
	<!-- Parameter to append the receiver of outbound message to the log file name -->
	<xsl:param name="dpconfig:receiver" select="on" />
	<dp:param name="dpconfig:receiver" type="dmToggle" required="true" xmlns="">
		<display>Add receiver</display>
		<description>Appends the name of the intended receiver to the log file name</description>
		<default>on</default>
	</dp:param>
	...
	<xsl:template match="/">
		...
		<xsl:if test="$dpconfig:receiver = 'on'">
			<xsl:variable name="routing-url" select="dp:variable('var://service/routing-url')"/>
			<xsl:variable name="url" select="substring-after($routing-url,'//')"/>
			<xsl:variable name="receiverName" select="str:split($url, '/')"/>
			<xsl:choose>
				<xsl:when test="contains($receiverName, ':')">
					<dp:set-variable name="'var://context/log/destination'"
					value="concat(dp:variable('var://context/log/destination'), '-', str:split(str:split(substring-after(dp:variable('var://service/routing-url'),'//'),'/'),':'))"/>
				</xsl:when>
				<xsl:otherwise>
					<dp:set-variable name="'var://context/log/destination'"
					value="concat(dp:variable('var://context/log/destination'), '-', str:split(substring-after(dp:variable('var://service/routing-url'),'//'),'/'))"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
