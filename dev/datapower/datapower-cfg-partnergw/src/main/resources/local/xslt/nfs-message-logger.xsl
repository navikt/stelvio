...
<xsl:stylesheet version="1.0"
    ...
    exclude-result-prefixes="dp dpconfig dputils date env str xsd"
	>

	<xsl:output method="xml" />
	<dp:summary xmlns="">
		<operation>xform</operation>
		<description>Message Logging</description>
	</dp:summary>
	...
	<xsl:template match="/">
            ...
            <env:Envelope>
                <env:Body>
                    <log-entry>
                        ...
                        <transaction><xsl:value-of select="dp:variable('var://service/transaction-id')"/></transaction>
                        <type><xsl:value-of select="$dpconfig:LogCategory"/></type>
                        <level><xsl:value-of select="$dpconfig:LogPriority"/></level>
                        <authenticated-user><xsl:value-of select="dp:variable('var://context/WSM/identity/authenticated-user')"/></authenticated-user>
                        <client-service-address><xsl:value-of select="dp:variable('var://service/client-service-address')"/></client-service-address>
                        <frontside-url><xsl:value-of select="dp:variable('var://service/URL-in')"/></frontside-url>
                        <backside-url><xsl:value-of select="dp:variable('var://service/URL-out')"/></backside-url>
                        <wsdl-port-operation><xsl:value-of select="dp:variable('var://service/wsm/service-port-operation')"/></wsdl-port-operation>
                        <message><xsl:copy-of select="."/></message>
                    </log-entry>
                </env:Body>
            </env:Envelope>        
        </xsl:variable>
        ...
        <xsl:variable name="discard" select="dp:soap-call(dp:variable('var://context/log/destination'), $log-message)"/>
