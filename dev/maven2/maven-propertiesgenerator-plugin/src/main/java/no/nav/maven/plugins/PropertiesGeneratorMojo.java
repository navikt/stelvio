    ...
    throws MojoExecutionException
{
    ...
    try
    {
        ...
        java.io.InputStream is = new FileInputStream(environmentDir + "/" + environmentName + ".properties");
        Properties props = new Properties();
        props.load(is);
        Set keys = props.keySet();
        String key;
        for(Iterator iterator = keys.iterator(); iterator.hasNext(); context.put(key, props.get(key)))
        ...
        for(int j=0; j < tempTemplates.length; j++){
        	...
        	if(tempFile.isFile() && (tempName.endsWith(".vm") || tempName.endsWith(".properties")))
            {
                String newName;
                if(tempName.endsWith(".properties")){
                	...
                	if(tempArrayList.size()==0){
                		...
                		tempArrayList.add(tempArrayList.size()-1, tempFile);
