...
public class FlowPhaseListener implements PhaseListener {
			...
			try {
				...
				try {
					...
					logger.debug("Thread " + Thread.currentThread().getId() + 
							" locked flowexecution with key " + flowExecutionKey + 
							" for flow-id " + flowExecution.getDefinition().getId());
					...
					lock.unlock();
					logger.debug("Thread " + Thread.currentThread().getId() + 
							" unlocked flowexecution with key " + flowExecutionKey + 
							" due to an exception");
					throw e;
				}
				catch (Error e) {
	...
	private void cleanupResources(FacesContext context) {
		...
		String key = null;
		try {
			key = FlowExecutionHolderUtils.getFlowExecutionHolder(context).getFlowExecutionKey().toString();
		}
		catch(Exception e) {}
		
		FlowExecutionHolderUtils.cleanupCurrentFlowExecution(context, key);
