...
public class AccessVoterManager implements AccessDecisionManager,
	...
	/**
	 * Iterates through the attributes in a ConfigAttributeDefinition and
	 * creates a list of <code>AccessDecisionVoter</code>s based on these. If
	 * no voters are found an <code>AlwaysAffirmativeVoter</code> will be
	 * added to the list so that access is always granted to the secure object.
	 * 
	 * @param config
	...
	public void addDecisionVoters(ConfigAttributeDefinition config)
		...
		try {
			while (iterator.hasNext()) {
				...
				Class clazz = Class.forName(configAttribute.getAttribute(),
						true, Thread.currentThread().getContextClassLoader());

				if (isAccessDecisionVoter(clazz)) {
					AccessDecisionVoter voter = (AccessDecisionVoter) clazz
							.newInstance();
			...
			throw new AccessDecisionVoterNotFoundException(ex, configAttribute
					.getAttribute(), "Class '" + configAttribute
					+ "' not found");
		} catch (InstantiationException ine) {
			throw new AccessDecisionVoterNotFoundException(ine, configAttribute
					.getAttribute(), "Class '" + configAttribute
					+ "' cannot be instantiated."
					+ "It is either an interface or abstract class.");
		} catch (IllegalAccessException ile) {
			throw new AccessDecisionVoterNotFoundException(ile, configAttribute
					.getAttribute(), "Could not create an instance of class '"
					+ configAttribute + "'");
		}
	}

	/**
	 * Checks if the presented class is an <code>AccessDecisionVoter</code>
	 * implementation.
	 * 
	 * @param clazz
	 *            the class to check
	 * @return <code>true</code> if the class is an
	 *         <code>AccessDecisionVoter</code>.
	 */
	 ...
	 * {@link AccessDecisionVoter}s and grants access if an
	 * <code>AccessDecisionVoter</code> voted affirmatively or denies access
	 * if there is a deny vote. If a voter abstain from voting the next one in
	 * the list will be invoked.
	 * <p>
	...
	public void decide(Authentication authentication, Object object,
		...
		while (voterIterator.hasNext()) {
			AccessDecisionVoter voter = (AccessDecisionVoter) voterIterator
					.next();
			...
			if (result == AccessDecisionVoter.ACCESS_GRANTED) {
				...
				if (object instanceof ReflectiveMethodInvocation) {
					...
					throw new MethodAccessDeniedException(
							"Secure object is not a ReflectiveMethodInvocation.");
				}
			}
		}
	...
	/**
	 * Returns whether or not access should be granted if all voters abstain
	 * from voting.
	 * 
	 * @return <code>true</code> if this property is explicitly set,
	...
	/**
	 * Sets whether or not access should be granted if all voters abstain from
	 * voting.
	 * 
	 * @param allowIfAllAbstainDecisions
	 *            <code>true</code> if access should be granted,
	 *            <code>false</code> otherwise.
	 */
	 ...
	 *            the class to check.
	 * 
	 * @return <code>true</code> if all voters support the presented class,
	 *         <code>false</code> otherwise.
	 */
