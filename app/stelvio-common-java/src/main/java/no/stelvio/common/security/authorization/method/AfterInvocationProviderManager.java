...
public class AfterInvocationProviderManager implements AfterInvocationManager {
	 ...
	 * creates a list of <code>AfterInvocationProvider</code>s based on
	 * these.
	 * 
	 * @param config
		...
		try {
			while (iterator.hasNext()) {
				...
				if (isAfterInvocationProvider(clazz)) {
					AfterInvocationProvider provider = (AfterInvocationProvider) clazz
							.newInstance();
					this.providers.add(provider);
				}
			}
		} catch (ClassNotFoundException ex) {
			throw new AfterInvocationProviderNotFoundException(ex,
					configAttribute.getAttribute(), "Class '" + configAttribute
							+ "' not found");
		} catch (InstantiationException ine) {
			throw new AfterInvocationProviderNotFoundException(ine,
					configAttribute.getAttribute(), "Class '" + configAttribute
							+ "' cannot be instantiated."
							+ "It is either an interface or abstract class.");
		} catch (IllegalAccessException ile) {
			throw new AfterInvocationProviderNotFoundException(ile,
					configAttribute.getAttribute(),
					"Could not create an instance of class '" + configAttribute
							+ "'");
		}
	}
	 ...
	 *            the <code>Object</code> that was returned from the secure
	 *            object invocation
	 * 
	...
	/**
	 * Checks if the presented class is an <code>AfterInvocationProvider</code>
	 * implementation.
	 * 
	 * @param clazz
	 *            the class to check
	 * @return <code>true</code> if the class is an
	 *         <code>AfterInvocationProvider</code>, <code>false</code>
	 *         otherwise.
	 */
	public boolean isAfterInvocationProvider(Class clazz) {
		return AfterInvocationProvider.class.isAssignableFrom(clazz) ? true
				: false;
	}
		...
		if (this.providers != null) {
			...
			while (iter.hasNext()) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Evaluating " + attribute + " against "
							+ provider);
				}
