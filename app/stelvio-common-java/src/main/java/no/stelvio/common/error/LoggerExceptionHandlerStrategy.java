...
*/
        ...
        if (t instanceof ApplicationException || t instanceof SystemException) {
            ...
            return (Throwable) handleInternal(
                    new SystemException(
                            t, new String[]{t.getLocalizedMessage(), t.getClass().getName()}));
        }
    }
...
*/
    ...
    private String getMessage(int code, Object[] arguments) {
        Err error = (Err) errorMap.get(new Integer(code)); // TODO: not the correct approach in new version

        if (null == error) {
            // The error is not configured
            error = (Err) errorMap.get(new Integer(0)); // TODO: not the correct approach in new version
