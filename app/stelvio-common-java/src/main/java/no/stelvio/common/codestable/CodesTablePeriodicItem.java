...
/**
 * CodesTablePeriodicItem represents an item in a <code>CodesTable</code> with a valid period.
 * <p/>
 ...
 */
@MappedSuperclass
public abstract class CodesTablePeriodicItem<K extends Enum, V> extends AbstractCodesTableItem<K, V> {
	...
	/** Constructor for <code>CodesTablePeriodicItem</code>. Only available to subclasses. */
	 ...
	 */
	@Deprecated
	public CodesTablePeriodicItem(K code, V decode, Date fromDate, Date toDate, Locale locale, boolean isValid) {
	 ...
	 */
	@Deprecated
	public CodesTablePeriodicItem(K code, V decode, Date fromDate, Date toDate, boolean isValid) {
		super(code, decode, isValid);
		this.fromDate = fromDate;
		this.toDate = toDate;
	}
