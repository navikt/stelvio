...
public class DefaultCodesTable<T extends CodesTableItem<K, V>, K extends Enum, V> extends AbstractCodesTable<T, K, V> implements CodesTable<T, K, V> {
	...
	/** {@inheritDoc} */
	public V getDecode(Enum code) throws ItemNotFoundException, DecodeNotFoundException {
		return decode(code.name());
	}

	/** {@inheritDoc} */
	public V getDecode(Enum code, Locale locale) throws DecodeNotFoundException {
	...
	public String getDecode(Object code, Locale locale) {
		return (String) decode(code, locale);
	}
}