...
/**
 * This class is an implementation of a {@link MethodDefinitionSource} with the
 * only purpose of merging two different {@link MethodDefinitionSource}
 * implementations into one source represented by this class.
 * 
 * @author persondab2f89862d3, Accenture
 * @version $Id$
 */
public class MethodDefinitionSources implements MethodDefinitionSource,
		InitializingBean {
	...
	/**
	 * Requires that at least one {@link MethodDefinitionSource} is set in the
	 * bean context.
	 * 
	...
	public void afterPropertiesSet() throws Exception {
		if ((this.methodDefinitionMap == null)
			...
			throw new IllegalArgumentException(
					"At least one methodDefinitionSource is required.");
		}
	}

	/**
	 * Returns the merged ConfigAttributeDefinition that applies to a given
	 * secure object from the two configured MethodDefinitionSources. Returns
	...
	public ConfigAttributeDefinition getAttributes(Object object) {
		ConfigAttributeDefinition defAttributes = (this.methodDefinitionAttributes != null) ? this.methodDefinitionAttributes
				...
				: null;
		ConfigAttributeDefinition defMap = (this.methodDefinitionMap != null) ? this.methodDefinitionMap
				.getAttributes(object)
				: null;
	...
	/**
	 * Merges the second <code>ConfigAttributeDefinition</code> parameter into
	 * the first with no duplicates.
	 * 
	 * @param definition
	 *            the final merged ConfigAttributeDefinition
	 * @param toMerge
	 *            the ConfigAttributeDefinition to merge into the first
	 *            parameter
	 */
	 ...
	 *            a MethodDefintionSource.
	 */
	public void setMethodDefinitionMap(
			MethodDefinitionSource methodDefinitionMap) {
		this.methodDefinitionMap = methodDefinitionMap;
	}

	/**
	 * If available, all of the <code>ConfigAttributeDefinition</code>s
	...
	/**
	 * Indicates whether the <code>ObjectDefinitionSource</code>
	 * implementation is able to provide <code>ConfigAttributeDefinition</code>s
	...
	public boolean supports(Class clazz) {
		return this.methodDefinitionAttributes != null ? this.methodDefinitionAttributes
				.supports(clazz)
