...
public class IbanValidator {
	...
	static{
		 ...
		 tempCharToIntMap.put('A', 10); tempCharToIntMap.put('B', 11);
		 tempCharToIntMap.put('C', 12); tempCharToIntMap.put('D', 13);
		 tempCharToIntMap.put('E', 14); tempCharToIntMap.put('F', 15);
		 tempCharToIntMap.put('G', 16); tempCharToIntMap.put('H', 17);
		 tempCharToIntMap.put('I', 18); tempCharToIntMap.put('J', 19);
		 tempCharToIntMap.put('K', 20); tempCharToIntMap.put('L', 21);
		 tempCharToIntMap.put('M', 22); tempCharToIntMap.put('N', 23);
		 tempCharToIntMap.put('O', 24); tempCharToIntMap.put('P', 25);
		 tempCharToIntMap.put('Q', 26); tempCharToIntMap.put('R', 27);
		 tempCharToIntMap.put('S', 28); tempCharToIntMap.put('T', 29);
		 tempCharToIntMap.put('U', 30); tempCharToIntMap.put('V', 31);
		 tempCharToIntMap.put('W', 32); tempCharToIntMap.put('X', 33);
		 tempCharToIntMap.put('Y', 34); tempCharToIntMap.put('Y', 35);
	 ...
	 * 
	 * @param noSpaceIban Iban witout spaces
	...
	/**
	 * Performs a mod97 operation on a numeric string in accordance with ISO7064.
	 * Handles all string lengths as the algorithm devides the string into substrings that can be parsed into integers.¨
	 * Once 
	 * @param numericString
	...
	private static int mod97NumericString(String numericString){
		if(numericString.length() < 10){
			...
			while(true){
				...
				if(numericString.length() == toCharIndex) //Break loop after final mod calculation
					break;
				
				fromCharIndex = toCharIndex;				
				//If mod97+toCharIndex is greater than the length of the numeric string, use length of numeric string as toCharIndex
				toCharIndex = ( (toCharIndex+(mod97 < 10 ? 8 : 7)) > numericString.length() ) ? numericString.length() : toCharIndex + ((mod97 < 10) ? 8 : 7);
