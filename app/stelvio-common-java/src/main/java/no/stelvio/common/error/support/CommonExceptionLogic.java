...
public abstract class CommonExceptionLogic implements Serializable {
	...
	protected CommonExceptionLogic(Object[] templateArguments) {
		...
		String userIdLocal = null;
		String screenIdLocal = null;
		String processIdLocal = null;
		String transactionIdLocal = null;

		try {
			requestContext = RequestContextHolder.currentRequestContext();
			securityContext = SecurityContextHolder.currentSecurityContext();

			userIdLocal = securityContext.getUserId();
			screenIdLocal = requestContext.getScreenId();
			processIdLocal = requestContext.getProcessId();
			transactionIdLocal = requestContext.getTransactionId();
		} catch (IllegalStateException e) {
			// Could happen, but not fatal
			// TODO find a better way to handle this, maybe have a getXxxxContext() that returns null
		}

		init(SequenceNumberGenerator.getNextId("ErrorId"), userIdLocal, screenIdLocal,
				processIdLocal, transactionIdLocal, templateArguments, false);
	}
