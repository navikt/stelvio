...
public class CacheManagementTest extends AbstractDependencyInjectionSpringContextTests {
	...
	private String cacheableString = new String(initialString);
	...
	public void testCacheManagement() throws BeansException, Exception {
		...
		assertTrue("Specified cache ("+cacheName+") doesn't exist.", cacheManagement.getCacheManager().cacheExists(cacheName));
		
		// Check number of caches
		log.debug("DEBUG: Checking number of Caches...");
		assertTrue("There are no caches registered in the cachemanager.", cacheManagement.getCacheManager().getCacheNames().length >= 3);
