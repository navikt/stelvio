...
public class AccessVoterManagerTests {
	...
	/**
	 * Add several valid AccessDecisionVoters represented by Strings. Expected
	 * outcome: All will be added to the list.
	 */
	@Test
	public void testAddDecisionVoters1() {
		...
		config
				.addConfigAttribute(new SecurityConfig(
						"no.stelvio.common.security.authorization.method.MockAccessDeniedVoter")); // grant
		config
				.addConfigAttribute(new SecurityConfig(
						"no.stelvio.common.security.authorization.method.MockAccessGrantedVoter")); // deny
		config
				.addConfigAttribute(new SecurityConfig(
						"no.stelvio.common.security.authorization.method.MockAccessAbstainVoter"));
	...
	/**
	 * Add a mix of AccessDecisionVoters and other classes represented by
	 * Strings.
	 * <p>
	...
	/**
	 * Add a list of classes represented by Strings that doesn't implement the
	 * AccessDecsionVoter interface.
	 * <p>
	 * <b>Expected outcome:</b> Omits these classes and add one instance of the
	 * AlwaysAffirmativeVoter to the list.
	 * 
	 */
	@Test
	public void testAddDecisionVoters3() {
		...
		if (manager.getDecisionVoters().size() == 1) {
			assertTrue(AlwaysAffirmativeVoter.class.isAssignableFrom(manager
					.getDecisionVoters().get(0).getClass()));
		} else {
	...
	/**
	 * Add several AccessDecsionVoters and a class that does not exist, i.e. the
	 * string passed in is not an applicaple name of a class.
	 * <p>
	...
	public void testAddDecisionVoters4() {
		...
		config
				.addConfigAttribute(new SecurityConfig(
						"no.stelvio.common.security.authorization.method.MockAccessDeniedVoter"));
	...
	private TestingAuthenticationToken makeTestToken() {
		return new TestingAuthenticationToken("somebody", "password",
				new GrantedAuthority[] { new GrantedAuthorityImpl("ROLE_1"),
						new GrantedAuthorityImpl("ROLE_2") });
	}
	...
	public void testDecideVoterGrantsAccess() {
		...
		config
				.addConfigAttribute(new SecurityConfig(
						"no.stelvio.common.security.authorization.method.MockAccessGrantedVoter"));
