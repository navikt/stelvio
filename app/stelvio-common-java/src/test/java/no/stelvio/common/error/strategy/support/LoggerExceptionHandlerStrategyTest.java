...
public class LoggerExceptionHandlerStrategyTest {
	...
	public void loggerIsInitializedWithName() {
		LoggerExceptionHandlerStrategy leh = createInstanceToTest();
		leh.setLogName("TEST");
	...
	private Extractor createExtractorMock() {
		...
		context.checking(new Expectations() {
			{
				allowing(extractor).messageFor(with(a(Throwable.class)));
	...
	private ErrorDefinitionResolver createErrorResolverMock() {
		...
		context.checking(new Expectations() {
			{
				allowing(errorDefinitionResolver).resolve(with(a(Throwable.class)));
