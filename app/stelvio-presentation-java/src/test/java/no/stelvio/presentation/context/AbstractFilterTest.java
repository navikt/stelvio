	...
	private FilterChain createFilterChain(boolean expectChainToProceed) throws IOException, ServletException {
		...
		if (expectChainToProceed) {
			context.checking(new Expectations() {
				{
					allowing(chain).doFilter(with(a(ServletRequest.class)), with(a(ServletResponse.class)));
				}
			});
		} else {
			context.checking(new Expectations() {
				{
					never(chain).doFilter(with(a(ServletRequest.class)), with(a(ServletResponse.class)));
				}
			});
		}
		return chain;
