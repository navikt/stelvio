 ...
 * <code>true</code>. This attribute should be set to <code>false</code> if the list of codes table items should not be
 * sorted. </li>
 * <li><code>showCodeValue</code> optional attribute, if true the code value will be shown instead of decode value in
 * drop-down.</li>
 ...
 * <li><code>showValidOnly</code> optional atttribute, can be set to <code>false</code> if one whishes to display codeitems
 * that are invalidated and expired. Defaults to true, displaying only codeitems that are valid. </li>
 * </ul>
 * <br>
...
public class CodesTableItemSelectOneMenu extends HtmlSelectOneMenu {
	...
	private Class resolveClass(String className, String attribute) {
		...
		try {
			...
			throw new InvalidArgumentException(className + "." + attribute, cnfe);
		}
	...
	/**
	 * Retrives the codes table items from the codestable manager. Fetches only codeitems valid today, 
	 * unless the "showValidOnly" atttribute is set to false. Default is true for this attribute. 
	 * 
	 * @return koder
	 */
	...
	public String getCtiClass() {
		if (getValueBinding(ATTRIBUTE_CTI_CLASS) != null) {
			ctiClass = (String) getValueBinding(ATTRIBUTE_CTI_CLASS).getValue(FacesContext.getCurrentInstance());
		}
	...
	public String getDefaultValue() {
		if (getValueBinding(ATTRIBUTE_DEFAULT_VALUE) != null) {
			defaultValue = (String) getValueBinding(ATTRIBUTE_DEFAULT_VALUE).getValue(FacesContext.getCurrentInstance());
		}
		return defaultValue;
	}
	...
	public String getComparator() {
		if (getValueBinding(ATTRIBUTE_COMPARATOR) != null) {
			comparator = (String) getValueBinding(ATTRIBUTE_COMPARATOR).getValue(FacesContext.getCurrentInstance());
		}
	...
	public boolean isSorted() {
		if (getValueBinding(ATTRIBUTE_SORTED) != null) {
			sorted = (Boolean) getValueBinding(ATTRIBUTE_SORTED).getValue(FacesContext.getCurrentInstance());
		}
	...
	/**
	 * Return true if code value is shown.
	 * 
	 * @return the showCodeValue
	 */
	public boolean isShowCodeValue() {
		if (getValueBinding(ATTRIBUTE_SHOW_CODE_VALUE) != null) {
			showCodeValue = (Boolean) getValueBinding(ATTRIBUTE_SHOW_CODE_VALUE).getValue(FacesContext.getCurrentInstance());
		}
	...
	public boolean isShowValidOnly() {
		if (getValueBinding(ATTRIBUTE_SHOW_ONLY_VALID_CODEITEMS) != null) {
			showValidOnly = (Boolean) getValueBinding(ATTRIBUTE_SHOW_ONLY_VALID_CODEITEMS).getValue(
	...
	/**
	 * Set value of showValidOnly.
	 * 
	 * @param showValidOnly showValidOnly
	 */
