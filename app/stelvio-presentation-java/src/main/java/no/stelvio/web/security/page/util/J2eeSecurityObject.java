...
import no.stelvio.web.security.page.parse.SecurityConfigurationXML;
 ...
 /**
  * This class reads in and maintains the security definitions from the <code>SecurityConfiguration</code> interface 
  * (the concrete implementation <code>SecurityConfigurationXML</code> is set to default if no other source is specified) 
  * It provides methods which can handle authorization of these security constraints against JEE-security and methods
  * which can handle a protocol switch for secure channel communication.
  * 
...
public class J2eeSecurityObject 
{
    ...
    private JSFApplication        secureJsfApplication = null;  //object that holds the secured page definitions
   ...
   * @param config the SecurityConfiguration
   */
  ...
  /**
   * Sets the SecurityConfiguration source, if not set a <code>SecurityConfigurationXML</code> 
  ...
  /**
   * Initializes the Security Object by obtaining the security definitions from the 
   * <code>SecurityConfiguration</code> source. If no such source is set a new 
   * <code>SecurityConfigurationXML</code> will be created as default.
   * 
   * @throws PageSecurityFileNotFoundException  if no configuration file is found when using the default 
   ...
   */
   public void initializeSecurityDefinitions() {
      ...
      if (this.securityConfiguration == null) {
      	  String configFile = exctx.getInitParameter(Constants.SECURITY_CONFIG_FILE);	
      	  try{
  ...
  /**
   * Shared method used to determine JSF page authorization requirements and if a user
   * should be allowed to access a page. Users that are not authenticated will be enforced to
   * authenticate if the page requires authentication. 
   *
   * @param viewId the requested page
   * @param session the HttpSession
   * @return <code>true</code> if the user is granted page access.
   * @throws PageAccessDeniedException if access to a page is denied.
   * @throws PageAuthenticationFailedException if the authentication process fails, i.e. cannot redirection fails.
   */
  public boolean authorizePageAccess(String viewId, HttpSession session) 
      ...
      pageRequiresAuthentication = pageRequiresAuthorization || page.requiresAuthentication();
      // if SSL mode should be kept after first time the application switches to SSL
      // then SSL communication is enforced an all subsequent pages, independently of
      ...
      if (pageRequiresAuthentication) {
    	...
    	if(log.isDebugEnabled()){
      	  log.debug("User is authenticated in authorization of " + viewId + " : " + authenticated + "");
        }
        if (authenticated && pageRequiresAuthorization){
          
        	boolean userIsAuthorized = pageRequiresAuthorization ? checkUserAuthorization(page) : true;
        	if(log.isDebugEnabled()){
            	  log.debug("User is authorized to access page " + viewId + " : " + userIsAuthorized);
              }
        	if(userIsAuthorized){
  ...
  * authentication servlet. The authentication servlet performs container managed
  * authentication and redirects the request back to the viewId it was contacted by.
  *
  * @param viewId the viewId of the requested page
  * @param session the HttpSession used to store the viewId of the requested page.
  * @return <code>true</code> if the user is authenticated
  * @throws IOException if the redirection fails.
  */
  private boolean authenticateRequest(String viewId,HttpSession session)throws IOException{
    ...
    if (usr == null) {
        ...
        String params = ((HttpServletRequest)exctx.getRequest()).getQueryString()!= null ? 
        				"?" + ((HttpServletRequest)exctx.getRequest()).getQueryString() : "";
        String originPageURI =  vh.getActionURL(FacesContext.getCurrentInstance(), viewId)+ params; 
       
        if(log.isDebugEnabled()){
      	  log.debug("User must be authenticated. Return URL upon successful authentication is "+originPageURI);
        }
        session.setAttribute(Constants.JSFPAGE_TOGO_AFTER_AUTHENTICATION, originPageURI);
        
        String j2eeContextPath = ((HttpServletRequest)exctx.getRequest()).getContextPath();
        
	    // if page requires SSL and authentication, the J2EE logon is requested using 
	    // https as well                 
	    if (this.pageRequiresSSL && 
	       ...
	       String authenticationEnforcerURL = "https://" + getHostNameFromRequest() + ":" + 
	                                          port + j2eeContextPath + "/jsfauthentication";
	       if(log.isDebugEnabled()){
	       	  log.debug("Authentication Servlet request URL is "+authenticationEnforcerURL);
	       }
  ...
  /**
  * Determines the security constraints like, authentication, authorization and
  * SSL requirements for a viewId. This object also knows about the required J2EE security roles
  * for page access and whether the role evaluation should be AND or OR. All security
  * information and the page ViewId are exposed as methods.
  *
  ...
  */
  ...
  {                                  
    ...
    if (page == null) {
        ...
        if(log.isDebugEnabled()){
	       	  log.debug("Performing wildcard search for security configuration of "+viewId);
	    }
        ...
        for (int i = wildcardAuthorization.length - 1; i > -1; i--) {
            ...
            if (page != null) {
            	if(log.isDebugEnabled()){
      	       	  log.debug("Wildcard search, found security configuration for "+protectedViews);
            	}
                break;
            }
        }
  ...
  /**
  * Determines if the authenticated user is member of the required J2EE roles. The
  * J2EE roles can be ORed or ANDed together. The normal J2EE container managed
  * security uses OR. AND requires the user to be a member of all the listed roles
  * to be authorized for page access
  *
  * @param page the JSFPage object containing the security constraints for one page
  ...
  */
  private boolean checkUserAuthorization(JSFPage page) {
      ...
      Iterator<J2EERole> rolesIterator =  j2eeRoles.getRoles() != null ? j2eeRoles.getRoles().iterator() : null;
      
      boolean authorized = false;
      boolean concatenateRolesWithOR = j2eeRoles.getRoleConcatenationType() != Constants.J2EE_ROLE_CONCATINATION_AND ? true : false;
  ...
  *
  * @param viewId id of the requested page
  * @param pageRequiresSSL true if the page requires SLL
  * @throws PageProtocolSwitchFailedException if redirecting from one protocol to the other fails.
  */
  public void handleProtocolSwitch(String viewId, boolean pageRequiresSSL)
  {
    ...
    if(log.isDebugEnabled()){
     	  log.debug("Page requires SSL = "+pageRequiresSSL+", channel is secure = "+isSecureSSLChannel+", is keep SSL = "+this.isKeepSSLMode);
	}
    ...
    if (pageRequiresSSL && !isSecureSSLChannel) {
      ...
      try{	  
    	  ...
    	  throw new PageProtocolSwitchFailedException(io,viewId,pageRequiresSSL,this.httpsPort);
      }
    }
  ...
  * @throws MalformedURLException if getRequestURL is malformed
  */
