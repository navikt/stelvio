 ...
 * server side page forwards to perform navigation. This JavaServer Faces PhaseListener
 * uses container managed authentication and authorization to solves some of the security
 * issues in J2EE
 * </p>
 * <li> It allows fine grained authorization and authentication including secure channel</li>
 * <li> An application can switch between SSL and non-SSL usage for each individual page</li>
 * <li> If required, a flag can be set so that once SSL is enabled it is kept for all pages</li>
 ...
 * </pre>
 * <p>
 * The phaselistener supports security configuration through an XML file where the security constraints
 * for all pages are defined. A page is registrered with its complete path relative to 
 * public_html, e.g. /protected/start.jsp. Wildcards are supported at the end of a path. 
 * To protect all pages in a directory "protected", the page entry in the security configuration 
 * file needs to be /protected/*.
 * </p>
 * <br>
 * <p>
 * Pages are protected by J2EE roles, where one or many roles can be specified for a
 * JSF page. The role named can be ORed or ANDed together, which is defined in the
 * security configuration file.
 * 
 * Note that all security roles used in the configuration file must either exist in the
 * web.xml deployment descriptor or mapped to other roles in this file. Please refer to
 * the J2EE Servlet specification for how to configure security roles.
 * </p>
 * <br>
 * <p>
 * Authentication is enforced by a redirect to an authentication servlet that needs to be
 * configured in the web.xml configuration file. If a user is already authenticated then
 * the authentication servlet is not called. The authentication servlet allows developers
 * to specify a page as to require authentication but no authorization, which is a common
 * scenario.
 * </p>
 * <br>
 * <p>
 ...
 * <br>
 * <br>
 * The phaselistener is configured to listen to any phase but page authorizations 
 * are only checked before the RENDER_RESPONSE phase and after the INVOKE_APPLICATION and RESTORE_VIEW
 * phases.
 * 
 * <br>
 * <br>
...
public class J2eeSecurityPhaseListener implements PhaseListener
{
	...
	private static final Log log = LogFactory.getLog(J2eeSecurityPhaseListener.class);
	private J2eeSecurityObject j2eeSecurityObject = null; 
	private boolean useSessionPageCache = true;
  
	public J2eeSecurityPhaseListener(){ }

	  /**
	   * Handle a notification that the processing for a particular phase of 
	   * the request processing lifecycle is about to begin.
	   * Responds to the <code>RENDER_RESPONSE</code> phase by 
	   * conducting the authorization of a specific JSF page. 
	   * 
	   * The JSF 1.2 specification states that all exceptions thrown in a phaselistener's before
	   * or after phase methods are caught, logged and swallowed. This means that any exceptions
	   * thrown in a phaselistener will not propagate beyond the afterPhase() and beforePhase() methods.
	   * For this reason this method will catch all exceptions thrown or propagated to it and pass them on 
	   * to a custom error-handler.
	   * 
	   * @param phaseEvent represents the beginning of processing for a particular phase 
	   ...
	   * Responds to the <code>RESTORE_VIEW</code> and <code>INVOKE_APPLICATION</code> phases by 
	   * conducting the authorization of a specific JSF page.
	   * 
	   * The JSF 1.2 specification states that all exceptions thrown in a phaselistener's before
	   * or after phase methods are caught, logged and swallowed. This means that any exceptions
	   ...
	   * to a custom error-handler.
	   *  
	   * @param phaseEvent represents the ending of processing for a particular phase 
	   * of the request processing lifecycle.
	   */
	   ...
	   * which this listener is interested in processing <code>PhaseEvent</code> events. 
	   * Legal values are the singleton instances defined by the <code>PhaseId</code> class, 
	   * including <code>PhaseId.ANY_PHASE</code> to indicate an interest in being 
	   * notified for all standard phases.
	   * 
	   * @return the id of the phase this listener is interested in, 
	   * which is <code>PhaseId.ANY_PHASE</code>
	   */
	  ...
	  /**
	   * Checks if the page needs to be evaluated and initiate the  
	   * handling of the security requirements. If a user is authorized to view a page, 
	   * this page will be cached as long as caching is turned on. 
	   * Used in both the <code>beforePhase()<code> 
					...
					if(pageAuthorized){
		               ...
		               if(userPageCache != null){
		            	   ...
		            	   if (log.isDebugEnabled()) {
		           			log.debug("Storing viewId "+viewId+" as authorized view with SSL requirement = "+requiresSSL);
		           		   }
	   ...
	   * 
	   * @param viewId the viewId of the current page
	   */
	  private boolean checkPageCache(String viewId) {
		  
		  ExternalContext exctx = FacesContext.getCurrentInstance().getExternalContext();
		  String initParam = exctx.getInitParameter(Constants.USE_PAGE_CACHE);		  
		  this.useSessionPageCache = initParam != null ? 
				  ( initParam.equalsIgnoreCase("FALSE")? false : true ) : true;
		  HashMap userPageCache = getUserSessionPageAuthorizationCache(this.useSessionPageCache);
	   ...
	   * 
	   * @param cacheEnabled page authorization cache is/not enabled
	   * @param viewId the id of a page
	   * @param userPageCache the authorized pages cache
	   * @return <code>true<code> if the page is found in the cache 
	   */
	  private boolean isPageInCache(boolean cacheEnabled, String viewId, HashMap userPageCache)
	  {
		  if(!cacheEnabled){
			  return false;
		  } 
		  //check if page was successfully authorized before
	      // if the page was authorized then the HashTable returns "true" or "false" indicating if the
	      ...
	      // if page was authorized before then we don't need to access and parse the security object. This improves
	      // performance, assuming that a user page authorization doesn't change while working within the application.
	      if(pageAlreadyAuthorized) {
	   ...
	   * and returned.
	   * 
	   * @param enabled page authorization cache is/not enabled
	   * @return the page autorization cache, <code>null</code> if the cache is not enabled 
	   */
	  private HashMap getUserSessionPageAuthorizationCache(boolean enabled)
	  {
	   if (enabled){
	        ...
	        HashMap userPageCache = (HashMap)sessionScopeMap.get("userAccessCache");
	        if (userPageCache == null){
	   ...
	   * 
	   * @throws PageAccessDeniedException if access is denied
	   * @throws PageAuthenticationFailedException if authentication redirection has failed
	   * @throws PageSecurityFileNotFoundException if security config file cannot be found
	   * @throws PageProtocolSwitchFailedException if protocol switch has failed
	   */
	  private boolean handleSecurity(PhaseEvent phaseEvent) 
	  {
	    ...
	    if (viewId != null ){
	        ...
	        boolean authorizedUser = false;
	       	...
	       	authorizedUser = this.j2eeSecurityObject.authorizePageAccess(viewId, session);
	        	 
	        if (authorizedUser) {  
	          session.setAttribute(Constants.PREVIOUS_JSF_PAGE_URL,viewId);
	          JSFPage page = this.j2eeSecurityObject.getPageObject(viewId);
	          this.j2eeSecurityObject.handleProtocolSwitch(viewId,page.requiresSSL());
	          return true;
	        }
	      }
	      return true;
	 ...
	 * 
	 * @param statusCode the HttpServletResponse status code.
	 * @param error_message the error message
	 */
	private void handleError(int statusCode, String error_message){
		  	
		try{
			HttpServletResponse response = 
				  (HttpServletResponse)FacesContext.getCurrentInstance().getExternalContext().getResponse();
			if(error_message != null){
				response.sendError(statusCode, error_message);
			}else{
				response.sendError(statusCode);
			}
		}catch(IOException doNothing){
    		log.error("SEVERE! Unexpected exception caught when redirecting to error page.'", doNothing);
    	}	  
	 }
	/**
	 ...
	 * @param event the PhaseEvent used to obtain information about current request.
	 */
	private void exceptionHandler(Exception e, PhaseEvent event){
		...
		if(e instanceof PageSecurityFileNotFoundException){
			...
			if(log.isDebugEnabled()){
        		log.debug("Could not find configuration file with security definitions. Sending HTTP error.");
        	}
        	handleError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e.getMessage());
			...
			if(log.isDebugEnabled()){
        		log.debug("User is not authorized to view the page. Sending HTTP error.");
        	}
        	handleError(HttpServletResponse.SC_FORBIDDEN, e.getMessage());
			...
			log.error("Unexpected exception caught at: '" 
				 	+ J2eeSecurityPhaseListener.class
				 	+ "' in phase '"
				 	+ event.getPhaseId() + "'.", e);
