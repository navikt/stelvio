...
/**
 * Class that acts as glue between Spring Batch and Stelvio Batch execution semantics. Uses {@link JobOperator},
 * {@link JobExplorer} and {@link JobRepository} to interact with Spring Batch framework.
 * 
...
public class DefaultSpringBatchJobOperator implements SpringBatchJobOperator {
	...
	public boolean stopBatch(String jobName) {
		try {
			return jobOperator.stop(getRunningExecutionId(jobName));
		} catch (Exception e) {
	...
	/**
	 * Get running execution id.
	 * 
	 * @param jobName
	 *            job name
	 * @return id
	 * @throws NoSuchJobException
	 *             if no job with jobname is running
	 */
	private Long getRunningExecutionId(String jobName) throws NoSuchJobException {
		Set<Long> executions = jobOperator.getRunningExecutions(jobName);
		if (logger.isDebugEnabled()) {
			logger.debug("executions:" + Arrays.toString(executions.toArray()));
		}
		if (executions.size() > 1) {
			throw new RuntimeException("More than 1 running batch with jobName=" + jobName);
		}
