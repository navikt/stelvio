 ...
 * A group-map with mappings from authentication levels to ldap groups determines which 
 * groups that will be added to the <code>Subject</code>.
 * 
...
public class WebsphereSubjectMapper implements SubjectMapper{
	...
	/**
	 * Creates a <code>Subject</code> from the supplied StelvioPrincipal. Groups from the user-registry 
	 * corresponding to the principal's authentication level will be added to the Subject using the 
	 * mappings in the group-map.
	 * @see {@link #buildSubject(String, List)}
	 * @see {@link #getLdapGroups(String[])}
	 * @see {@link #getGroupsFromMap(String)}
	 * 
	 * @param principal the StelvioPrincipal to create to Subject from
	 * @return a Subject containining the userid of the StelvioPrincipal and groups corresponding to the
	 * principal's authentication level.
	...
	protected Subject buildSubject(StelvioPrincipal principal, List<String> ldapGroups ){
		...
		String user_id = principal.getUserId();
		...
		hashtable.put(AttributeNameConstants.WSCREDENTIAL_SECURITYNAME, user_id);	
	...
	/**
	 * Makes a lookup in the userregistry for the supplied groups and returns
	 * the unique groupId for each group as a List.
	 * 
	 * @param groups the groups to lookup.
	 * @return the unique groupId for each of the groups found in the userregistry.
	 * @throws EntryNotFoundException if a userregistry operation fails
	 * @throws NamingException if a userregistry operation fails
	 * @throws CustomRegistryException if a userregistry operation fails
	 * @throws RemoteException if a userregistry operation fails
	 */
	...
	throws EntryNotFoundException, NamingException, CustomRegistryException, RemoteException {
		...
		for (String group : groups) {
			String ldapGroup = ad_reg.getUniqueGroupId( group.trim() );
