...
/**
 * StelvioTai is a TrustAssociationInterceptor implementation which is a single-sign-on mechanism that
 * will be triggered when an unauthenticated user attempts to access a protected resource. When this 
 * happens the interceptor can examine the values in the request to determine if it should create
 * a Subject and authenticate the user with the application server. The criteria for authentication are
 * based upon userid and authentication level, that is it is the authentication level which determines 
 * which user-registry groups that will be added to the Subject. 
 * 
...
public class StelvioTai implements TrustAssociationInterceptor {
	...
	/**
	 * Initializes the interceptor. This involves using properties set in the WebSphere admin console and
	 * initialization of the components used by the interceptor.
	 * Currently the only available property is: debug = [ true | false ], which is false by default.
	 *  
	 * @param props the properties set in the WebSphere admin console
	 * @return a number indicating the success of the initialization, 0 is ok.
	 * @throws WebTrustAssociationFailedException if the initialization fails.
	 */
	...
	/**
	 * Initializes and sets the interceptor's StelvioAccessManager, SSORequestHandler and SubjectMapper
	 * components by using a StelvioTaiPropertiesConfig to read in properties from properties files.  
	 * 
	 * @see StelvioAccessManager
	 * @see SSORequestHandler
	 * @see SubjectMapper
	...
	/**
	 * Initializes and sets the interceptor's StelvioAccessManager, SSORequestHandler and SubjectMapper
	 * components by using a StelvioTaiConfig.
	 *   
	...
	/**
	 * Authenticates the user by creating a Subject based on the authentication data from the request. 
	 * The authentication data will be extracted from the request using the SSORequestHandler, transformed into 
	 * a StelvioPrincipal using the StelvioAccessManager and mapped into a Subject along with the corresponding 
	 * LDAP groups using the SubjectMapper. If the authentication data is incomplete or invalid an Exception will
	 * be thrown.
	 * 
	 * @param req the current request
	 * @return a Subject containing information about the current user.
	 * @throws Exception in these cases: 
	 * <li>PrincipalNotValidException</li> - the principal representation could not be converted into a StelvioPrincpal.
	 * <li>PrincipalNotValidException</li> - the StelvioPrincipal could not be mapped to a Subject
	 * <li>PrincipalNotValidException</li> - could not get the required authentication data from the request
	 * <li>Exception</li> - if an exception occurs while creating a Subject using the SubjectMapper, typically vendor specific 
	 * exceptions.
	 */
	...
	/** 
	 * This method is invoked when an unauthenticated request attempts to access a protected resource.
	 * It determines whether or not this interceptor should process the current request by using 
	 * the {@link SSORequestHandler}'s actOnRequest() method.
	 * 
	 * @param req the current request
	 * @return true if this interceptor should process the request, false otherwise.	 
	 * @throws WebTrustAssociationException if there is a situation where noe definite response can be 
	 * given.
	 */ 
	...
	/** 
	 * This method is invoked after the isTargetInterceptor() method and is used to extract the 
	 * information that WebSphere needs to create a securitycontext for the current user. It will
	 * use the SSORequestHandler to extract the authentication data, the StelvioAccessManager to
	 * transform this data into a StelvioPrincipal and the SubjectMapper to create a Subject that
	 * will be returned in a TAIResult object to the server. The TAIResult object also contains a
	 * status code that will inform WebSphere if the authentication data is valid.
	 * 
	 ...
	 * @throws WebTrustAssociationFailedException if the authentication process fails.
	 */	
