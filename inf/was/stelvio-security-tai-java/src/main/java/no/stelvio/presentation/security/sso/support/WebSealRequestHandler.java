...
/**
 * An implementation of the SSORequestHandler which by default targets requests from WebSEAL. It can determine whether the
 * current request should be acted on and get the representation of a userprincipal depending on the values present in the
 * request. When requests are routed through WebSEAL the authentiction data is placed in http headers, so by default the
 * RequestValueType, i.e. the type of value to retrieve from the request, is set to RequestValueType.HEADER. However, this
 * implementation can also make its decisions on other request value types. This can be achieved by setting the RequestValueType
 * property and can come in handy especially in environments where there is no WebSEAL instance. This implementation also
 * supports the use of PAC headers from WebSEAL. PAC is an IBM properitary encoded data-structure and is represented in the
...
public class WebSealRequestHandler implements SSORequestHandler {
    ...
    /**
     * Enumeration representing the keys for the entries in the configuration for the request values to extract from the
     * request.
     */
    /*
     * private enum ConfigEntry { PAC, ORIGINAL_USER_NAME, AUTHENTICATION_LEVEL, ACCESS_MANAGER_USER; }
     */
    /**
     ...
     */
    /*
     * public Properties getRequestValueKeyConfig() { return requestValueKeyConfig; }
     */
    /**
     * Sets the configuration for the names of the request values to extract from the request.
     * 
     * @param requestValueKeyConfig
     *            the requestValueKeyConfig to set.
     */
    /*
     * public void setRequestValueKeyConfig(Properties requestValueKeyConfig) { this.requestValueKeyConfig =
     * requestValueKeyConfig; }
     */
    /**
     * Decides whether the current request should be acted on based on whether or not the request contains values for the id of
     * the access-manager user and the original user. If these values are present and the access-manager userid equals the
     * access-manager userid configured, the method will return true or otherwise false.
     * 
     * @param request
     *            the current request
    ...
    /**
     * Extracts request values representing a user's authentication data from the current request and returns a userprincipal
     * representation. Unless PAC headers are used this will be a <code>DefaultStelvioPrincipal</code> object containing the
     * userid and authentication level of a user. If PAC headers are to be used the userprincipal representation will be
     * returned as a <code>byte[]</code> object.
     * 
     * @param request
     *            the current request
     * @return a userprincipal representation of type StelvioPrincipal or byte[] if request values are present, otherwise
     *         <b>null</b>.
     ...
     *            the current request
     * @param entry
     *            the configuration entry for the value to check
    ...
    /**
     * Private helper method which casts the supplied object to a String. If the object is not of type String the object's
     * toString() method will be returned unless the object is null. If the object is null an empty String will be returned.
     * 
     * @param value
     *            the object to cast
    ...
    /**
     * Private helper method which extracts a request value specied by the configuration entry parameter as a String.
     * 
     * @param request
     *            the current request
     * @param entry
     *            the configuration entry for the value to extract.
    ...
    /**
     * Private helper method to extract a request value from the current request using a configuration entry.
     * 
     * @param request
     *            the current request.
     * @param entry
    ...
    /**
     * Private helper method which extracts the value specified by the key parameter from the HttpSession.
     * 
     * @param request
     *            the current request
     * @param key
     *            the key for the value to extract
    ...
    /**
     * Private helper method which extracts the attribute specified by the key parameter from the current request.
     * 
     * @param request
     *            the current request
     * @param key
    ...
    /**
     * Private helper method which extracts the request-parameter specified by the key from the current request.
     * 
     * @param request
     *            the current request
     * @param key
    ...
    /**
     * Private helper method which extracts the http header value specified by the key from the current request.
     * 
     * @param request
     *            the current request
     * @param key
     ...
     * 
     * @param entry
     *            the entry to extract from the configuration
     * @return the value for the configuration entry.
     */
    /*
     * private String getConfigEntry(ConfigEntry entry){ return this.requestValueKeyConfig.getProperty(entry.name()) != null ?
     * this.requestValueKeyConfig.getProperty(entry.name()).trim() : null; }
     */
    /**
     * Specifies if the request-handler should use PAC headers which is an encoded data-structure sent out from WebSEAL.
     * 
    ...
    /**
     * Gets the type of value to extract from the request. Default is RequestValueType.HEADER.
     * 
    ...
    /**
     * Sets the type of value to extract from the request. Default is RequestValueType.HEADER.
     * 
     * @param requestValueType
     *            the requestValueType to set.
     */
     ...
     *            the userid of the access-manager user.
     */
    ...
    /**
     * @param requestValueKeys
     *            the requestValueKeys to set
     */
