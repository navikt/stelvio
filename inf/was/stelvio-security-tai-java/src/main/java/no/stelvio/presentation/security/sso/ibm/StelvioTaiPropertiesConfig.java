 ...
 * use setter methods to create <code>StelvioAccessManager, SSORequestHandler and SubjectMapper</code>
 * objects. These can be retrieved with getter methods. If no of the setters for the above mentioned 
 * objects have been used, two properties files will be used to create these objects.
 * There should be one file for the common properties of the TAI module and one with
 * the authentication levels as numbers with corresponding LDAP groups. Default names for these files
 * are <code>stelvio-tai-common.properties</code> and <code>stelvio-tai-ldap-groups.properties</code>, 
 * however these can be changed using setters.
 * 
 * For the common properties file the following keys are valid:
 * 
 * <li>REQUEST_VALUE_TYPE = Specifies what type of value which can be extracted from a request.
 *  Valid values are: [HEADER, PARAMETER, ATTRIBUTE, SESSION_ATTRIBUTE]
 * </li>
 * <li>ACCESS_MANAGER_USER_NAME = The username of the access-manager user.</li>
 * <li>ORIGINAL_USER_NAME = The name of the original username value to be retrieved from the 
 * request.</li>
 * <li>AUTHENTICATION_LEVEL = The name of the authentication level value to be retrieved from the 
 * request.</li>
 * <li>ACCESS_MANAGER_USER = The name of the access-manager user value to be retrieved from the 
 * request.</li>
 * <br/>
 * <br/>
 * For the LDAP groups file the following keys are valid:
 * <br/>
 * <li>1 = [a ldap group for authentication level one]</li> 
 * <li>2 = [a ldap group for authentication level two]</li>
 * <li>3 = [a ldap group for authentication level three]</li>
 * <li>4 = [a ldap group for authentication level four]</li>
 * <br/>
 * <br/>
 * 
 * 
...
public class StelvioTaiPropertiesConfig implements StelvioTaiConfig{
	...
	/**
	 * Gets the StelvioAccessManager. If no access-manager is present a <code>WebSealAccessManager</code>
	 * will be created. 
	 * 
	...
	/**
	 * Gets the SSORequestHandler. If no request-handler is present a <code>WebSealRequestHandler</code>
	 * will be created from the common properties file.
	 *  
	...
	/**
	 * Gets the SubjectMapper. If no subject-mapper is present a <code>WebsphereSubjectMapper</code>
	 * will be created from the ldap groups properties file.
	 * 
	...
	/**
	 * Private helper method which creates a WebsphereSubjectMapper from the properties in
	 * the ldap groups properties file.
	 * 
	 ...
	 * 
	 * @param commonPropertiesFile the filename of the common properties file.
	 */
	 ...
	 * 
	 * @param ldapGroupsPropertiesFile the filename for ldap groups properties file.
	 */
	 ...
	 * 
	 * @param accessManager the access-manager to set.
	 */
	 ...
	 * 
	 * @param subjectMapper the subject-mapper to set.
	 */
	...
	/**
	 * Private helper method for matching the string for the REQUEST_VALUE_TYPE property
	 * with the enum RequestValueType. 
	 * 
	 * @param props the common properties file
	 ...
	 *  
	 * @param props the properties containing the property to trim.
	 * @param key the key of the property to trim.
	 ...
	 * 
	 * @param file the filename of the properties file to load
