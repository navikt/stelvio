...
/**
 * An implementation of the SSORequestHandler which by default targets requests from openAM. It can determine whether the
 * current request should be acted on and get the representation of a userprincipal depending on the values present in the
 * request. When requests are routed through openAm the openam-sessionid is placed in a cookie, so by default the
 * RequestValueType, i.e. the type of value to retrieve from the request, is set to RequestValueType.COOKIE.
 * 
